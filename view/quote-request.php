...
<main class="w-100 flex-1 d-flex flex-column overflow-auto w-100 bg-150 shadow-sm rounded-3 p-3">
    <h1><?= $title ?></h1>
    ...
    <div class="flex-1 d-flex flex-column justify-content-center align-items-center">
        ...
        <?php else : ?>
            <form class="bg-100 shadow-sm rounded-3 p-3" method="POST" action="index.php?p=quote-request">
                ...
                <div class="mb-3">
                    ...
                    <input type="tel" class="form-control" id="telephone" name="telephone" placeholder="01 23 45 67 89" required>
                </div>
                <div class="<?= $vehicle ? "" : "d-none" ?> mb-3">
                    <label for="vehicle" class="form-label">Véhicule</label>
                    <input class="form-control" type="text" id="vehicle" name="vehicle" value="<?= $vehicle ? $vehicle['brand'] . " " . $vehicle['model'] . " (" . $vehicle['entry_year'] . ") - " . formatNumber($vehicle['price']) . " €" : "" ?>" disabled readonly>
                </div>
                <div class="mb-3">
